...
export class CamplNgxHeaderComponent implements OnInit, OnDestroy {
  menu_open = false;
  search_open = false;
  mega_menu_open = false; // this is the desktop mega_menu
  mega_menu_target = ''; // allows us to place ngClass on targets
  ...
  public myidsearch = 'CamplNgxHeaderComponentSearch';
  public myidnav = 'CamplNgxHeaderComponentNav';
  public myidmega = 'CamplNgxHeaderComponentMega';
  ...
  toggleBodyNav() {
    const id: string = !this.menu_open ? this.myidnav : 'NOTSET';
    this.primary_comp.sendId(id);
  }

  toggleSearch() {
    const id: string = !this.search_open ? this.myidsearch : 'NOTSET';
    this.primary_comp.sendId(id);
  }
